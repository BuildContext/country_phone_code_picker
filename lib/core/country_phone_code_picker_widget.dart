import 'package:country_phone_code_picker/core/country_flag.dart';
...
class _CountryPhoneCodePickerState extends State<CountryPhoneCodePicker> {
  ...
  void initState() {
    ...
    // CountryController _countryController = Get.put(widget.countryController);
    // _countryController.updateSelectedCountry(widget.defaultCountryCode);
    Get.find<CountryController>()
        .updateSelectedCountry(widget.defaultCountryCode);
    super.initState();
  }
  ...
  ImageProvider getImage(CountryController controller) {
    ImageProvider image;
    try {
      image = AssetImage(
        'flags/${controller.selectedCountryCode.toLowerCase()}.png',
        package: 'country_phone_code_picker',
      );
    } catch (_) {
      print('get flag from network - e ${_}');
